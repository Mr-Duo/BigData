...
async function fetchUrlsRecursively(url, maxDepth, currentDepth, stream, baseDomain) {
    ...
    try {
        ...
        for (const childUrl of childUrls) {
            ...
            if (childDomain === baseDomain) {
                await fetchUrlsRecursively(childUrl, maxDepth, currentDepth + 1, stream, baseDomain);
            }
        }
    } catch (error) {
        console.error(`Error processing URL ${url} at depth ${currentDepth}:`, error);
    }
}
...
app.post('/fetch-html', async (req, res) => {
    ...
    if (typeof depth !== 'number' || depth < 1) {
        return res.status(400).json({ url: url, error: 'A valid maxDepth is required' });
    }
    ...
    try {
        ...
        const timestamp = Date.now();
        stream.push(JSON.stringify({ url, timestamp, error: 'Failed to process URL' }) + '\n');
    }
